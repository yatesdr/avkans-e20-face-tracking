...
def track_single_without_zoom(cam_ip):
    ...
    pid_pan_params = (0.5,0.02,0.)
    pid_tilt_params = (0.5,0.02,0.)
    ...
    while(True):
        ...
        cam_abs_pos = ptz.ptz_get_abs_position() # About 50ms or so usually.
        ...
        if (abs(error_vector[0]) > permitted_error[0] or abs(error_vector[1]) > permitted_error[1]):
            ...
            ptz.send_raw(ptz.cmd.ptz_to_abs_position(cam_correct_pos[0],cam_correct_pos[1],pan_speed,tilt_speed))
            #ptz.wait_complete()
...
def load_sources_from_db():
    with app.ctx.db_env.begin(write=False) as txn:
    ...
    if not sources:
        print("Sources invalid or None, creating empty source list...")
        sources=list()
        store_sources_to_db(sources)

    else:
        print("Sources is valid: ",sources)
...
def store_sources_to_db(sources):
    ...
    with app.ctx.db_env.begin(write=True) as txn:
...
async def addcambyip(request):
     ...
     return text(f"{camip} {camname}")


@app.get("/camlist")
async def camlist(request):
    sources=load_sources_from_db()
    return json(sources)

@app.get("/removesource")
async def removesource(request):
    sources=load_sources_from_db()
...
async def getPreview(request):
    ...
    return text(html_src_tag)




@app.get("/startTracking")
async def startTracking(request):
    ...
    if cam_ip is None or idx is None:
        return text(f"Failure - No cameras found by that identifier: idx={idx} cam_ip={cam_ip}")
    
    # Spawn separate thread for tracking code
    sources[idx]['haltTrackingThread']=False

    T = threading.Thread(target=track_single_without_zoom,args=(cam_ip,))
    T.start()
...
async def stopTracking(request):
...
@app.get("searchndi")
async def searchndi(request):
    cam=AvkansCamera()
    found=cam.get_sources_as_dict()
    return text(jsonlib.dumps(found,indent=2))

if __name__ == "__main__":

    # Use lightning db for inter-worker communication
    print("Opening database!!! ")
    app.run(host="0.0.0.0",port=8091,debug=True,auto_reload=True,reload_dir=["./","./lib","./html"])
